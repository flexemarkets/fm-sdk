...
class OrderUtilsTest {
    ...
    private static Order order(long id, long original, long supplier, String type) {
        return new Order(null, null, id, original, supplier, null, type, Order.SIDE_BUY,
                10L, 100L, null, 8L, 1L, 300L, "STK", 11L, null, null);
    }

    @Test
    void anOrderThatIsItsOwnOriginalAndSupplierWasSubmitted() {
        assertThat(OrderUtils.isSubmit(order(5, 5, 5, Order.TYPE_LIMIT))).isTrue();
    }

    @Test
    void aSplitOrTradeCarriesTheIdItCameFromAndWasNot() {
        assertThat(OrderUtils.isSubmit(order(9, 5, 5, Order.TYPE_LIMIT)))
                .as("a child of order 5")
                .isFalse();
        assertThat(OrderUtils.isSubmit(order(9, 9, 5, Order.TYPE_LIMIT)))
    ...
    void aCancelCountsAsASubmission() {
        assertThat(OrderUtils.isSubmit(order(9, 5, 5, Order.TYPE_CANCEL))).isTrue();
    }
}
