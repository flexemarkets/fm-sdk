/**
 * OrderSide and OrderType in TypeScript, where a literal union makes the type nearly
 * free.
 *
...
import { OrderType, OrderSide, toOrderType, toSide } from "../src/types.ts";
...
test("a member is an ordinary string", () => {
  ...
  assert.equal(JSON.stringify({ side: OrderSide.BUY }), '{"side":"BUY"}');
});
...
test("contra pairs the two sides", () => {
  assert.equal(contra(OrderSide.BUY), "SELL");
  assert.equal(contra("sell"), "BUY", "a plain string still works");
});

test("the helpers take either a member or a loose string", () => {
  assert.ok(isBuy(OrderSide.BUY) && isBuy("buy"));
  assert.ok(isSell("SELL") && !isSell(OrderSide.BUY));
