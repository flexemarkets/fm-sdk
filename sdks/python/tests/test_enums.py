"""OrderSide and OrderType in Python, where StrEnum makes the type nearly free.
...
it -- ``"BUY" is OrderSide.BUY`` is False -- which is why the SDK coerces at the
boundary with ``OrderSide.of`` instead of assuming an enum arrived.
"""
...
from fm.enums import OrderType, OrderSide
...
def test_a_member_is_a_string():
    ...
    assert json.dumps({"side": OrderSide.BUY}) == '{"side": "BUY"}'


def test_a_side_is_read_whatever_its_casing():
    assert OrderSide.of("BUY") is OrderSide.BUY
    assert OrderSide.of("buy") is OrderSide.BUY
    assert OrderSide.of(" Sell ") is OrderSide.SELL


def test_an_unknown_value_is_none_rather_than_a_raise():
    assert OrderSide.of("BYU") is None
    assert OrderSide.of(None) is None
    assert OrderType.of("MARKET") is None, "the server has sent this"


def test_contra_pairs_the_two_sides():
    assert OrderSide.BUY.contra() is OrderSide.SELL
    assert contra("sell") is OrderSide.BUY, "a plain string still works"


def test_the_helpers_take_either_an_enum_or_a_string():
    assert is_buy(OrderSide.BUY) and is_buy("buy")
    assert is_sell("SELL") and not is_sell(OrderSide.BUY)
