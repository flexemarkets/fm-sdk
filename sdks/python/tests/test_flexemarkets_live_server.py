...
def test_opens_a_desk_on_the_endpoint_marketplace_without_throwing():
    fm = Flexemarkets.connect(client_description="fm-sdk-live-test/desk")
    try:
        marketplace_id = fm.endpoint_marketplace_id
        desk = fm.desk(marketplace_id)
        try:
            assert desk.marketplace_id == marketplace_id
            assert len(desk.markets) > 0, "marketplace should have at least one market"
            for m in desk.markets:
                book = desk.book(m.id)
...
def test_shares_one_desk_across_repeat_calls_for_same_marketplace():
    fm = Flexemarkets.connect(client_description="fm-sdk-live-test/sharedDesk")
    try:
        marketplace_id = fm.endpoint_marketplace_id
        a = fm.desk(marketplace_id)
        b = fm.desk(marketplace_id)
        try:
            assert a.marketplace_id == b.marketplace_id
            assert a.markets == b.markets
            # Closing one handle must NOT close the shared desk — `b`
