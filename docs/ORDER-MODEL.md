...
| you want | use |
|---|---|
| the last trade, and who took it | `MarketView.trades(marketId)` → `Trades.last()` → `Trade.aggressor` |
| both sides of every recent trade | `Trades.mostRecentTrades()` → `Trade.resting` / `Trade.aggressor` |
...
- **The trade snapshot's order is not fixed.** `/v1/orders/recent-trades`
  ...
  after it. `Trades` sorts by time either way; if you read the snapshot list
...
| the pairing rule, in prose and in code | `Trade` / `Trades` in each SDK |
